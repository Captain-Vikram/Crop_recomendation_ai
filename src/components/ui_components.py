...
def create_minimal_sidebar():
    """
    Create a minimal sidebar with essential information and improved local AI display
    """
    st.sidebar.title("🌱 Plant Recommendation AI")
    
    # Use enhanced Local AI Manager
    try:
        from components.local_ai_manager import LocalAIManager
    ...
    # Show model status with enhanced information
    if "Local AI" in ai_choice:
        if lm_studio_available:
            ...
            try:
                ...
                if models:
                    ...
                    if active_model:
                        model_info = manager.get_model_info_display(active_model)
                        st.sidebar.markdown(f"**Active:** {model_info['type']}")
                        if model_info['is_specialized']:
                            st.sidebar.success("✅ Specialized for Agriculture")
                        ...
                        with st.sidebar.expander("📋 Select Model"):
                            selected = st.selectbox(
                                "Choose model:",
                                models,
                                index=models.index(active_model) if active_model in models else 0,
                                key="sidebar_model_selector",
                                help="Select which model to use for recommendations"
                            )
                            if selected != active_model:
                                st.session_state.selected_local_model = selected
                    else:
        ...
        st.sidebar.markdown("**Type:** Cloud-based AI")
    ...
    # Show Gemini quota information
    st.sidebar.markdown("---")
    st.sidebar.markdown("### 📊 API Status")
    
    if "Gemini" in ai_choice:
        st.sidebar.warning("⚠️ Using Gemini API (Free Tier)")
        with st.sidebar.expander("ℹ️ Quota Limits"):
            st.markdown("""
            **Gemini Free Tier Limits:**
            ...
            1. **Use Local AI** - Switch to LM Studio (unlimited)
            2. **Upgrade Plan** - [Get paid Gemini access](https://ai.google.com)
            3. **Wait** - Free tier resets daily
            """)
    else:
        st.sidebar.success("✅ Using Local AI (No Quota Limits)")
        st.sidebar.info("Local AI processing is unlimited and private!")
